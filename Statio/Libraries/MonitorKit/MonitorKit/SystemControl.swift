...
enum SystemControl {
    ...
    static var hostName: String { try! SystemControl.string(for: [CTL_KERN, KERN_HOSTNAME]) }

    /// e.g. "x86_64" or "N71mAP"
    /// NOTE: this is *corrected* on iOS devices to fetch hw.model
    static var machine: String {
        #if os(iOS) && !arch(x86_64) && !arch(i386)
            return try! SystemControl.string(for: [CTL_HW, HW_MODEL])
        #else
            return try! SystemControl.string(for: [CTL_HW, HW_MACHINE])
        #endif
    }
    ...
    static var model: String {
        #if os(iOS) && !arch(x86_64) && !arch(i386)
            try! SystemControl.string(for: [CTL_HW, HW_MACHINE])
        #else
            try! SystemControl.string(for: [CTL_HW, HW_MODEL])
        #endif
    }

    static var activeCPUs: Int32 { try! SystemControl.value(ofType: Int32.self, forKeys: [CTL_HW, HW_AVAILCPU]) }

    static var osRelease: String { try! SystemControl.string(for: [CTL_KERN, KERN_OSRELEASE]) }

    static var osType: String { try! SystemControl.string(for: [CTL_KERN, KERN_OSTYPE]) }

    static var osVersion: String { try! SystemControl.string(for: [CTL_KERN, KERN_OSVERSION]) }

    static var version: String { try! SystemControl.string(for: [CTL_KERN, KERN_VERSION]) }

    #if os(macOS)
        /// e.g. 199506 (not available on iOS)
        static var osRev: Int32 { try! SystemControl.value(ofType: Int32.self, forKeys: [CTL_KERN, KERN_OSREV]) }

        /// e.g. 2659000000 (not available on iOS)
        static var cpuFreq: Int64 { try! SystemControl.value(ofType: Int64.self, forName: "hw.cpufrequency") }

        /// e.g. 25769803776 (not available on iOS)
        static var memSize: UInt64 { try! SystemControl.value(ofType: UInt64.self, forKeys: [CTL_HW, HW_MEMSIZE]) }
    #endif
}
